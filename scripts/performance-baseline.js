/*
  ...
  - Posts to /v1/onboarding with minimal payload, then polls /v1/plans/status?jobId=...
...
async function oneRun(i){
  ...
  try {
    const onboarding = await fetch(`${GATEWAY_URL}/v1/onboarding`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer dev-token' },
    ...
    while (polls++ < MAX_POLLS && status !== 'completed' && status !== 'failed'){
      await sleep(POLL_MS);
      const resp = await fetch(`${GATEWAY_URL}/v1/plans/status?jobId=${encodeURIComponent(jobId)}`, {
