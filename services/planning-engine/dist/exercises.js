...
export function getExercisesByEquipment(availableEquipment) {
    return EXERCISE_LIBRARY.filter(ex => ex.equipment.some(eq => availableEquipment.includes(eq)));
}
export function getExercisesByDifficulty(difficulty) {
    return EXERCISE_LIBRARY.filter(ex => ex.difficulty === difficulty);
}
export function getRecommendedExercises(proficiency, equipment, category) {
    ...
    const allowedDifficulties = difficultyMap[proficiency] || ['beginner'];
    exercises = exercises.filter(ex => allowedDifficulties.includes(ex.difficulty));
    return exercises;
}
export function getExerciseSubstitutes(exerciseId, availableEquipment) {
    const original = EXERCISE_LIBRARY.find(ex => ex.id === exerciseId);
    if (!original)
        return [];
    return EXERCISE_LIBRARY.filter(ex => ex.id !== exerciseId &&
