...
function sha256(value: string): string { return crypto.createHash('sha256').update(SALT + value).digest('hex'); }
...
export function filterAndSanitizeContext(ctx: LogContext | undefined) {
  ...
  for (const [k, v] of Object.entries(ctx)) {
    ...
    if (typeof v === 'string') { const s = sanitizeText(v); out[k] = s.text; hashes.push(...s.hashes); } else { out[k] = v; }
  }
