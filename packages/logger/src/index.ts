...
export interface Logger { debug: (msg: string, ctx?: LogContext) => void; info: (msg: string, ctx?: LogContext) => void; warn: (msg: string, ctx?: LogContext) => void; error: (msg: string|Error, ctx?: LogContext & { error?: unknown }) => void; }
export type CreateLoggerOptions = { service?: string; env?: string; module?: string; sampler?: { info: number; debug: number } };
function baseEvent(level: LogLevel, service: string, moduleName: string, env: string, msg: string, ctx?: LogContext, errObj?: Error): LogEvent {
  const { text, hashes } = sanitizeText(msg); const { context, hashes: ctxHashes } = filterAndSanitizeContext(ctx);
  const evt: LogEvent = { level, msg: text, ts: new Date().toISOString(), service, module: moduleName || 'unknown', env, context, pii_hashes: [...hashes, ...ctxHashes] };
  if (errObj) { evt.error = { name: errObj.name, message: errObj.message, stack: errObj.stack }; }
  return evt;
}
