...
export class ContractMetricsService {
  ...
  private readonly ALLOWED_LABELS = {
  ...
  private readonly MAX_VALUE_LENGTH = 50;
   ...
   * Ensures no PII in labels and enforces allowlist
   */
  recordLegacyMapping(field: string, value: string, environment: string = 'unknown'): void {
    // Validate against allowlist
    if (!this.isLabelAllowed('field', field)) {
      console.warn(`⚠️ Disallowed field label: ${field}`);
      return;
    }
    
    if (!this.isLabelAllowed('value', value)) {
      console.warn(`⚠️ Disallowed value label: ${value}`);
      return;
    }
    
    if (!this.isLabelAllowed('environment', environment)) {
      console.warn(`⚠️ Disallowed environment label: ${environment}`);
      return;
    }
  ...
  recordValidationFailure(type: string, reason: string, environment: string = 'unknown'): void {
    // Validate against allowlist
    if (!this.isLabelAllowed('field', 'validation_type')) {
      return;
    }
  ...
  recordDriftDetection(severity: 'low' | 'medium' | 'high', environment: string = 'unknown'): void {
    // Validate against allowlist
    if (!this.isLabelAllowed('field', 'drift_severity')) {
      return;
    }
   ...
   */
  private isLabelAllowed(labelType: keyof typeof this.ALLOWED_LABELS, value: string): boolean {
    return this.ALLOWED_LABELS[labelType].includes(value);
  }
  
  /**
   * Sanitize label values to prevent PII leakage and high cardinality
   */
  private sanitizeLabel(value: string): string {
    ...
    const sanitized = value
      ...
      .substring(0, this.MAX_VALUE_LENGTH) // Hard cap length
      .toLowerCase();
    
    // Additional high-cardinality protection
    if (sanitized.length > 20 && !this.isLabelAllowed('value', sanitized)) {
      // Truncate further for non-allowlisted values
